...
e1_estimation_data <- e1_data_no_gather %>%
  ...
  group_by(condition, adj) %>%
  summarise_at(c("chosetarget", "choselure", "choseother"), sum) %>%
  mutate(utt = if_else(adj, "blue dax", "dax")) %>%
  select(-adj)

  select(condition, adj, item, chose_n)


size_parameter <- webppl(program_file = "webppl/infer_descrete_semantics", 
                          data = mean_data, 
